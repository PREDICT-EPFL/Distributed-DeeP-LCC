% =========================================================================
%               Moderate-scale Traffic Simulation
%               15 Vehicles with 5 CAVs
%               The head vehicle has a sinusoidal perturbation
%
...
data_number     = 1;    % set to 100 for random simulations
% Data set
...
for i_data = 1:data_number
    ...
    load(['_data\trajectory_data_collection\',ID_str,'_data',data_str,'_',num2str(i_data),...
    ...
    for k = Tini:total_time_step-1
        ...
        if mix
            switch controller_type  
                case 1
                    tic
                    % Calculate control input via DeeP-LCC
                    if constraint_bool
...
for i = 1:n_vehicle
    if ID(i) == 1
            plot(begin_time:Tstep:end_time,S(begin_time/Tstep:end_time/Tstep,i+1,2),'Color',mymap_red_white_blue(108-(id_cav-1)*8,:),'linewidth',line_width); hold on; % line for velocity of CAVs
            id_cav  = id_cav+1;
    end
end
grid on;
...
for i = 1:n_vehicle
    if ID(i) == 1
            plot(begin_time:Tstep:end_time,S(begin_time/Tstep:end_time/Tstep,i,1)-S(begin_time/Tstep:end_time/Tstep,i+1,1),'Color',mymap_red_white_blue(108-(id_cav-1)*8,:),'linewidth',line_width); hold on; % line for velocity of CAVs
            id_cav  = id_cav+1;
    end
end
grid on;
...
if output_bool
    if mix
        save(['_data\simulation_data\',ID_str,'_',num2str(i_data),'_ControllerType_',num2str(controller_type),'_PerType_',num2str(per_type),'_T_',num2str(T),'.mat'],...
            'hdv_type','acel_noise','S','T','Tini','N','ID','Tstep','v_star','computation_time','iteration_num');
    else
        save(['_data\simulation_data\HDVs','_PerType_',num2str(per_type),'.mat'],...
